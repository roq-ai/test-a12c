...
import { getHabits } from 'apiSdk/habits';
import { HabitInterface } from 'interfaces/habit';

import { HabitListPage as ShowCaseList } from 'pages/habits';

function DashboardPage() {
  const { session } = useSession();

  const habitStat = useSWR<PaginatedInterface<HabitInterface>>(
    () => `/habits`,
    () => getHabits({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Habits',
      result: habitStat?.data?.totalCount,
      isLoading: habitStat.isLoading,
    },
  ];
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`habits`} passHref legacyBehavior>
                  <Button
